...
void setup() {
  ...
  Serial.begin(2000000); //   115200   250000
//  Serial.begin(9600); 
 ...
 // Wire.setClock(400000); //Устанавливает тактовую частоту обмена данными по I2C интерфейсу. https://all-arduino.ru/biblioteki-arduino/arduino-biblioteka-wire/
...
// Serial.print("________________status=========");
...
void loop() {
  ...
  //time_c = micros()- time_0; // есть также в цикле read_write_sensor_ADC1015()
...
//  Serial.print("_________________________________________ATH=");
  ...
  if( (Devices_authorization.SHT31_present_44==true) and (Devices_presents.SHT31_present_44==true) and (time_SHT>=2000) )
  {
   // 
...
void setting_timere(){
    ...
    OCR1A = 35335; // установка регистра совпадения  300Гц  ((1/300Гц) / (1/16МГц) -1)

    TCCR1B |= (1 << WGM12);  // включить CTC режим 
    TCCR1B |= (1 << CS10); // Установить биты на коэффициент деления = без деления
    TCCR1B |= (0 << CS11);
    TCCR1B |= (0 << CS12);
...
void get_current_time_micros(){
  time_00=micros();
  flag_resettintg_timer = false;
  if (time_00<time_0) {
    time_0=time_00;
    flag_resettintg_timer=true;
  }
  time_c = micros()- time_0;
 // return flag_resettintg_timer time_c 
}
...
//    
  get_current_time_micros();  // set time_c, flag_resettintg_timer
  //time_c = micros()- time_0;
...
////  float val = analogRead(1);    // считываем напряжение с аналогового входа
 ...
 // Serial.println("____________________________time");
...
void read_sensors_ATH_x38()
{
  cli();
    ...
    if (flag_resettintg_timer == false){
      buf_hum_temp_x38.crc = 0; // ? crc8_bytes((byte*)&buf, sizeof(buf) - 1);
    } else{
      buf_hum_temp_x38.crc = 1;
    }
    //buf_hum_temp_x38.crc=0;
    cod_signal=8;
    Serial.write((byte*)&cod_signal, sizeof(cod_signal));
  ...
  my_Devices.nDevices_I2C=0;
    ...
    for(address = 1; address < 127; address++ ){
        ...
        if (error == 0){
         ...
         //       Serial.println("Finde ADS1115 x48");
             // Serial.print(address,HEX);
             // delay(1000);
            };
            if (address == 0x38) {
             ...
             // Serial.print(address,HEX);
               my_Devices.nDevices_I2C++;
           //    Serial.println("Finde AHT25 x38");
             // delay(1000);
            };
            if (address == 0x44) {
             ...
             // Serial.print(address,HEX);
               my_Devices.nDevices_I2C++;
           //    Serial.println("Finde SHT31 x44");
             // delay(1000);
            };
