...
//  delay(5000);
 ...
 cli();  // отключить глобальные прерывания
    ...
    OCR1A = 65532; // установка регистра совпадения  300Гц  ((1/300Гц) / (1/16МГц) -1)
...
//  buf_float_temperature.name_tag=7;
  ...
  float_1 = ads1015.readADC_Differential_2_3(); // Измеряем значение АЦП на канало 2-3
   time_c3 = micros()- time_c3;
 Serial.print("_____________________________timeADC mksec =");
...
//   // Расчёт напряжения
  ...
  buf_float_temperature.val_1 = 0;//float_1*k_ads1015;//k_ads1015;
...
////  float val = analogRead(1);    // считываем напряжение с аналогового входа
  ...
  buf_float_temperature.val_R_1 =0;//float_0*k_ads1015; //val/1024 *1.1/47*1000;
//
//  //sinhron=123;
...
//
  ...
  if( (Devices_authorization.AHT25_present_38==true) and (Devices_presents.AHT25_present_38==true) and (time_AHT>=8000) )
  {
   // 
...
void setting_timere(){
    ...
    OCR1A = 20332; // установка регистра совпадения  300Гц  ((1/300Гц) / (1/16МГц) -1)
...
ISR(TIMER1_COMPA_vect)
{
 ...
 // float_1 =ads1015.readADC_Differential_2_3(); // Измеряем значение АЦП на канало 2-3
  cli();  
  time_c = micros();
  unsigned long dt=time_c-time_0;
